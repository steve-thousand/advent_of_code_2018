input = """[1518-10-05 00:10] falls asleep
...
unix_start = datetime(1518,1,1) #lol
records = []
...
for i in input.split("\n"):
    ...
    if m:
        ...
        epoch = (date - unix_start).total_seconds()

        record = {}
        record["raw_date"] = raw_date
        record["epoch"] = epoch
        record["date"] = date
        record["id"] = m.group(3)
        record["action"] = m.group(4)
        records.append(record)

def sortEpoch(val):
    return str(val["epoch"])

records.sort(key=sortEpoch)

guards = {}
most_frequent_minutes = {}
most_frequent_user_per_minute = {}
most_frequent_minute = 0
most_visits_in_a_minute = 0
most_sleep = 0
for i in records:
    if i["action"] == "begins shift":
        current_id = i["id"]
        if current_id not in guards:
            current_guard = {"id": current_id, "total": 0, "hours_slept": {}}
            guards[current_id] = current_guard
        else:
            current_guard = guards[current_id]
    elif i["action"] == "falls asleep":
        start_date = i["date"]
    elif i["action"] == "wakes up":
        end_date = i["date"]
        diff_time = end_date - start_date

        while start_date < end_date:

            if start_date.minute not in most_frequent_minutes:
                most_frequent_minutes[start_date.minute] = 0
                
            if start_date.minute not in current_guard["hours_slept"]:
                current_guard["hours_slept"][start_date.minute] = 0
            current_guard["hours_slept"][start_date.minute] = current_guard["hours_slept"][start_date.minute] + 1

            if current_guard["hours_slept"][start_date.minute] > most_frequent_minutes[start_date.minute]:
                most_frequent_minutes[start_date.minute] = current_guard["hours_slept"][start_date.minute]
                most_frequent_user_per_minute[start_date.minute] =  current_id
                if current_guard["hours_slept"][start_date.minute] > most_visits_in_a_minute:
                    most_visits_in_a_minute = int(current_guard["hours_slept"][start_date.minute])
                    most_frequent_minute = start_date.minute

            start_date += timedelta(0,60) #add minute
        
        minutes = diff_time.seconds / 60
        current_guard["total"] = int(current_guard["total"]) + minutes
        if current_guard["total"] > most_sleep:
            most_sleep = current_guard["total"]
            most_sleepy_guard = current_guard

def sortMostSleepy(val):
    return int(val["hours_slept"])

most_slept_per_hour = max(most_sleepy_guard["hours_slept"].values())
most_slept_hour = filter(lambda x:x[1] == most_slept_per_hour, most_sleepy_guard["hours_slept"].items())[0][0]
print("part 1: " + str(int(most_sleepy_guard["id"]) * int(most_slept_hour)))

print("part 2: " + str(int(most_frequent_minute) * int(most_frequent_user_per_minute[most_frequent_minute])))